...
onMounted(async () => {
  try {
    books.value = await collectionsApi.getCollectionBooks(collectionId)
  } finally {
    loading.value = false
  }
})
...
<template>
  <div>
    <button @click="$router.push('/collections')" class="text-sm mb-4 flex items-center gap-1" style="color: var(--text-secondary)">
      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
      返回收藏夹
    </button>
    <h1 class="text-2xl font-bold mb-6" style="color: var(--text-primary)">收藏夹详情</h1>
    ...
    <div v-else-if="books.length === 0" class="text-center py-12">
      <p style="color: var(--text-secondary)">此收藏夹暂无有声书</p>
    </div>

    <div v-else class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
      <router-link v-for="book in books" :key="book.id" :to="{ name: 'book-detail', params: { id: book.id } }"
